...
# Train the model
def train(model, criterion, optimizer, device, dataloader, total_paras):
...
def test(model, criterion, device, dataloader, total_paras):
    experiment_id = total_paras['experiment_id']
    seed = total_paras['seed']
    sequence_length = total_paras['sequence_length']
...
def train_lstm(config, checkpoint_dir=False):
    ...
    with open('LSTM_tune.yaml', 'r') as spec_file:
    ...
    for epoch in range(1000):
        train(model, criterion, optimizer, device, train_loader, total_paras)
        error = test(model, criterion, device, test_loader, total_paras)
        # Set this to run Tune.
        if epoch % 3 == 0:
            with distributed_checkpoint_dir(step=epoch) as checkpoint_dir:
                path = os.path.join(checkpoint_dir, "checkpoint")
                torch.save((model.state_dict(), optimizer.state_dict()), path)
        tune.report(mae=error)
...
def main():
    ...
    train(writer, model, criterion, optimizer, device, train_loader, total_paras)
    test(writer,  model, criterion, device, test_loader, total_paras)
    torch.save(model.state_dict(), 'runs/' + experiment_id + '/' + 'model.ckpt')
